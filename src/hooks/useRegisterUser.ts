...
const useRegisterUser = (): { registerUser: (user: User) => Promise<void>; error: string | null } => {
  ...
  const registerUser = async (user: User): Promise<void> => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(typeof errorData.message === 'string' ? errorData.message : 'Network response was not ok');
      }
